<?php
/**
 * Dashboard Liabilities API
 * Returns total liabilities breakdown for tenant company
...
try {
    ...
    // Get liability accounts with balances - Aligned with QUERIES.md
    // EXCLUDE external accounts (is_system_account = 1) - they are not company liabilities
    $sql = "SELECT
                a.id,
                a.account_code,
                a.account_name,
    ...
    respond(true, 'Liabilities loaded', $liabilities, 200);

} catch (Throwable $e) {
    error_log('Dashboard liabilities error: ' . $e->getMessage());
